...
function News() {
  const newsList = [
    {
      date: "2025",
      content: "Congratulations to Vishnu on the publication of his research paper titled- Phase-aware LSTM projections for learning from heterogeneous electroencephalogram montages",
  ...
  // Getting years sorted (latest first)
  const sortedYears = Object.keys(groupedNews).sort(
      ...
      {sortedYears.map((year) => (
        <div key={year} className="year-section">
          ...
          <ul className="news-list">
            {groupedNews[year].map((news, index) => (
